...
const AdminRequestedAppointments = () => {
  ...
  const fetchDataByIds = async (ids) => {
    try {
      ...
      const fetchPromises = ids.map(async (id) => {
        const response = await fetch(`${backendUrl}/appointments/id/${id}`);
        if (response.ok) {
  ...
  const fetchRequestedAppointments = async () => {
    try {
      ...
      if (policeId) {
        const response = await fetch(`${backendUrl}/police/reqappointments/${policeId}`);
  ...
  const handleAccept = async (id) => {
    try {
      ...
      await fetch(`${backendUrl}/appointments/confirm`, {
        method: "PUT",
        headers: {
